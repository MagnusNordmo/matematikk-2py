...
test("den selvstendige kontrollsiden inneholder hele banken", async () => {
  ...
  assert.equal(embedded.oppgaver.length, 915);
  assert.equal(embedded.groups, 60);
  assert.equal(embedded.oppgaver.filter((question) => question.del === 1).length, 462);
  assert.equal(embedded.oppgaver.filter((question) => question.del === 2).length, 453);
  assert.ok(html.includes("data:font/woff2;base64,"));
  assert.ok(html.includes('id="part-filters"'));
  assert.doesNotMatch(html, /\b(?:515|262|253) oppgaver\b/);
  assert.doesNotMatch(html, /\b50 Del 2-case\b/);
});

test("elevflaten viser ikke størrelsen på oppgavebanken", async () => {
  const source = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  assert.doesNotMatch(source, /\b(?:515|262|253) oppgaver\b/);
