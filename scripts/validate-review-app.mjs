...
assert(embedded.oppgaver.length === 970, "HTML-filen må inneholde 970 oppgaver.");
...
assert(new Set(embedded.oppgaver.map((question) => question.id)).size === 970, "Oppgave-ID-ene må være unike.");
...
assert(embedded.oppgaver.filter((question) => question.del === 1).length === 508, "Del 1 må ha 508 oppgaver.");
...
console.log("Kontroll bestått: 970 oppgaver, 60 Del 2-case, 11 temaer og alle originale tekster er bevart.");
