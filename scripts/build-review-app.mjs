...
const reviewBank = {
  ...
  oppgaver: bank.oppgaver.map((question) => ({
    ...question,
    hints: question.hint,
    vanskelighetsgrad: ({ 1: "lett", 2: "middels", 3: "utfordrende" })[question.niva],
...
console.log(JSON.stringify({ output: outputPath, questions: reviewBank.oppgaver.length,
  ...
  difficulty: Object.fromEntries(["lett", "middels", "utfordrende"].map((level) => [level, reviewBank.oppgaver.filter((question) => question.vanskelighetsgrad === level).length])) }));
