...
// Use the same representations as the student app in the standalone review.
const presentationSource = await readFile(join(projectDir, "app", "presentation.tsx"), "utf8");
...
const reviewBank = {
  ...
  oppgaver: bank.oppgaver.map((question) => ({
    ...
    renderedContext: renderContext(question.oppgavegruppe ? groupById.get(question.oppgavegruppe.id) : null) + renderContext(question),
    renderedChoices: (question.fasit.alternativer ?? question.fasit.valg?.alternativer ?? []).map(renderMathText),
...
<head>
  ...
  <style>
    ...
    .review-context{margin-top:16px;overflow-x:auto}.review-context table{border-collapse:collapse;width:100%;font-size:1rem}.review-context th,.review-context td{padding:8px;border:1px solid var(--line);text-align:left}.review-context pre{padding:16px;background:#eef2f3;white-space:pre;overflow:auto;font-size:1rem;line-height:1.6}.review-context svg{max-width:100%;height:auto}.review-context figure{margin:12px 0}.review-choices{line-height:1.7}
