...
const sourcePath = join(projectDir, "public", "oppgaver.json");
...
const reviewBank = {
  versjon: bank.versjon,
  temaer: bank.temaer,
  oppgaver: bank.oppgaver.map((question) => ({
    ...question,
    vanskelighetsgrad: question.vanskelighetsgrad,
    renderedSporsmal: renderMathText(question.sporsmal),
    renderedHints: question.hints.map(renderMathText),
...
<body>
  <header class="site-header"><div class="brand" aria-label="Matematikk 2P-Y"><span class="brand-mark">2P</span><span><strong>Matematikk 2P-Y</strong><small>Oppgaver og hint</small></span></div><span class="version">Oppgavebank ${escapeHtml(bank.versjon)}</span></header>
  <main>
    <section class="hero"><p class="eyebrow">Kontrollverktøy</p><h1>Se gjennom oppgavene, steg for steg.</h1><p class="lead">Her finner du alle oppgavene fra elevappen, sortert etter tema. Åpne en oppgave for å lese hintene i riktig rekkefølge og se fasiten. Vanskelighetsgraden er en grov vurdering for målgruppen i 2P-Y.</p><div class="summary-row" aria-label="Oppsummering"><span class="summary-chip">${bank.oppgaver.length} oppgaver</span><span class="summary-chip">${bank.oppgaver.reduce((sum, q) => sum + q.hints.length, 0)} hint</span><span class="summary-chip">${bank.temaer.length} temaer</span><span class="summary-chip">Lett · middels · vanskelig</span></div></section>
    <section class="filter-panel" aria-label="Filtrer oppgavene"><div class="filter-top"><p class="filter-label">Velg tema</p><span class="filter-status" id="filter-status" aria-live="polite"></span></div><nav class="topic-scroll" id="topic-filters" aria-label="Temaer"></nav><div class="difficulty-row" id="difficulty-filters"><span class="filter-label">Vanskelighetsgrad</span></div></section>
    <div id="results"></div>
    <footer>Oppgavene er laget for Del 1 uten hjelpemidler. Vanskelighetsmerkingen er veiledende og er ment som en enkel sortering, ikke en formell vurdering av eksamensnivå.</footer>
  </main>
  <script id="question-data" type="application/json">${serializedBank}</script>
  <script>
    (()=>{const bank=JSON.parse(document.getElementById("question-data").textContent),results=document.getElementById("results"),topicFilters=document.getElementById("topic-filters"),difficultyFilters=document.getElementById("difficulty-filters"),status=document.getElementById("filter-status"),levels=["alle","lett","middels","vanskelig"];let activeTopic="alle",activeLevel="alle";
    function el(tag,className,text){const node=document.createElement(tag);if(className)node.className=className;if(text!==undefined)node.textContent=text;return node}
    function filterButton(label,value,type,small=false){const button=el("button","filter-pill"+(small?" small":""),label);button.type="button";button.dataset.value=value;button.dataset.type=type;button.addEventListener("click",()=>{if(type==="topic")activeTopic=value;else activeLevel=value;updateActive();render()});return button}
    topicFilters.append(filterButton("Alle temaer","alle","topic"));bank.temaer.forEach(theme=>topicFilters.append(filterButton(theme.kortnavn,theme.id,"topic")));levels.forEach(level=>difficultyFilters.append(filterButton(level==="alle"?"Alle nivåer":level[0].toUpperCase()+level.slice(1),level,"level",true)));
    ...
    function updateActive(){document.querySelectorAll(".filter-pill").forEach(button=>{const active=button.dataset.type==="topic"?button.dataset.value===activeTopic:button.dataset.value===activeLevel;button.classList.toggle("active",active);button.setAttribute("aria-pressed",String(active))})}
    function questionCard(question,number){const article=el("article","question-card"),main=el("div","question-main");main.append(el("span","question-number",String(number)));const content=el("div","question-content"),meta=el("div","question-meta");meta.append(el("span","question-id","Oppgave "+question.id));meta.append(el("span","difficulty-badge "+question.vanskelighetsgrad,question.vanskelighetsgrad));content.append(meta);const questionText=el("div","question-text");questionText.innerHTML=question.renderedSporsmal;content.append(questionText);main.append(content);article.append(main);const details=el("details","question-details"),summary=el("summary","","Vis "+question.hints.length+" hint og fasit");details.append(summary);const solution=el("div","solution");solution.append(el("p","solution-label","Hint i rekkefølge"));const list=el("ol","hint-list");question.renderedHints.forEach(hint=>{const item=el("li","hint-item");item.innerHTML=hint;list.append(item)});solution.append(list);const answer=el("div","answer-box");answer.append(el("span","answer-label","Fasit"));const answerText=el("div","answer-text");answerText.innerHTML=question.renderedSvar;answer.append(answerText);solution.append(answer);details.append(solution);article.append(details);return article}
    function render(){results.replaceChildren();expandButton.textContent="Åpne alle viste";const visible=bank.oppgaver.filter(question=>(activeTopic==="alle"||question.tema===activeTopic)&&(activeLevel==="alle"||question.vanskelighetsgrad===activeLevel));status.textContent=visible.length+" av "+bank.oppgaver.length+" oppgaver vises";if(!visible.length){results.append(el("div","empty-state","Ingen oppgaver passer med disse filtrene."));return}bank.temaer.forEach(theme=>{const questions=visible.filter(question=>question.tema===theme.id);if(!questions.length)return;const section=el("section","theme-section");section.id="tema-"+theme.id;const heading=el("header","theme-heading");heading.append(el("span","theme-symbol",theme.symbol));const titleBlock=el("div");titleBlock.append(el("h2","",theme.navn));titleBlock.append(el("p","",theme.beskrivelse));heading.append(titleBlock);heading.append(el("span","theme-count",questions.length+" oppgaver"));section.append(heading);const list=el("div","question-list");questions.forEach((question,index)=>list.append(questionCard(question,index+1)));section.append(list);results.append(section)})}
    updateActive();render()})();
  </script>
</body>
</html>`;
...
console.log(JSON.stringify({ output: outputPath, questions: reviewBank.oppgaver.length,
  ...
  difficulty: Object.fromEntries(["lett", "middels", "vanskelig"].map((level) => [level, reviewBank.oppgaver.filter((question) => question.vanskelighetsgrad === level).length])) }));
